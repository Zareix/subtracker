...
function AdminPage() {
  ...
  const updateExchangeRatesMutation = useMutation({
    ...
    onSuccess: () => toast.success(m.admin_misc_update_exchange_rates_success()),
    onError: (err) => toast.error(err.message),
  })
  ...
  const importDataMutation = useMutation({
    mutationFn: (data: Parameters<typeof adminImportData>[0]["data"]) => adminImportData({ data }),
    ...
    <div className="grid max-w-[100vw] items-start gap-4">
      <section>
        <header className="flex flex-wrap items-center justify-between">
          <h1 className="text-3xl font-bold">{m.admin_users_title()}</h1>
          <CreateUserDialog />
        </header>
                  ...
                  <TableCell className="flex items-center justify-end gap-2">
                    {user.id !== session.data?.user.id && (
                      <>
                        ...
                        <EditUserDialog user={{ ...user, role: user.role as UserRole }} />
                      </>
                    )}
                  </TableCell>
                </TableRow>
      ...
      <section>
        <header className="flex items-center justify-between">
          <h1 className="text-3xl font-bold">{m.admin_misc_title()}</h1>
        </header>
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            <Button disabled={importDataMutation.isPending}>{m.admin_misc_import_data()}</Button>
            <Input
              ...
              onChange={async (e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                importDataMutation.mutate(
                  JSON.parse(text) as Parameters<typeof adminImportData>[0]["data"],
                )
              }}
              type="file"
              accept=".json"
