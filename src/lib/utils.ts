...
export function takeFirstOr<T extends unknown[], U>(values: T, defaultValue: U): T[number] | U {
  if (values.length > 0) {
...
export const takeFirstOrThrow = <T extends unknown[]>(values: T, error: string | Error) => {
...
>(
  ...
  if (sort === "NEXT_PAYMENT_DATE") {
    return sortedSubscriptions.sort((a, b) => compareAsc(a.nextPaymentDate, b.nextPaymentDate))
  }
...
>(
  ...
  if (filters.schedule) {
    ...
    filteredSubscriptions = filteredSubscriptions.filter((s) => s.schedule === filters.schedule)
  }
