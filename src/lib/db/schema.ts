...
import { index, int, integer, primaryKey, real, sqliteTable, text } from "drizzle-orm/sqlite-core"
...
export const subscriptions = sqliteTable(
  "subscription",
  {
    ...
    currency: text("currency", { length: 255 }).notNull().$type<Currency>().default("EUR"),
...
export const users = sqliteTable(
  "user",
  {
    ...
    role: text("role", { length: 255 }).notNull().$type<UserRole>().default("user"),
    image: text("image", { length: 255 }),
    baseCurrency: text("base_currency").$type<Currency>().default("EUR").notNull(),
