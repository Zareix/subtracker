import { env } from "~/env"
import { Currencies, type Currency, DEFAULT_BASE_CURRENCY } from "~/lib/constant"
...
export const updateExchangeRates = async () => {
  ...
  const response = await fetch(`${env.FRANKFURTER_API_URL}/v2/rates?${params}`, { method: "GET" })
  ...
  for (const currency of nonBaseSymbols) {
    const row = data.find((item) => item.base === DEFAULT_BASE_CURRENCY && item.quote === currency)
    const rate = row?.rate

    if (!rate || !Number.isFinite(rate)) {
      throw new Error(`Missing or invalid rate for ${currency}. Received: ${String(rate)}`)
    }
