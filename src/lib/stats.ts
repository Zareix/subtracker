import { addMonths, endOfMonth, isBefore, isSameMonth, isThisMonth } from "date-fns"
...
const getRetainPrice = (
  ...
  const base = filters.users ? subscription.price / subscription.users.length : subscription.price
...
export const getStats = (subscriptions: SubscriptionItem[], filters: Pick<Filters, "users">) => {
  const now = new Date()
  ...
  const thisMonthBreakdown = toBreakdown(
    subscriptions.filter(
      (s) => isThisMonth(s.nextPaymentDate) || isThisMonth(s.previousPaymentDate),
    ),
    () => 1,
  )
