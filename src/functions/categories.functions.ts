...
export const createCategory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const category = takeFirstOrThrow(
      await db.insert(categories).values({ name: data.name, icon: data.icon }).returning({
        id: categories.id,
...
export const deleteCategory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await runTransaction(db, async (tx) => {
      await tx.update(subscriptions).set({ category: 1 }).where(eq(subscriptions.category, data.id))
