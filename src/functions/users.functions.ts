...
export const getSocialProviders = createServerFn({ method: "GET" }).handler(async () => {
  ...
  if (env.OAUTH_ENABLED && env.OAUTH_PROVIDER_ID) {
    providers.push(env.OAUTH_PROVIDER_ID)
  }
  return providers
})
...
export const editUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (session.user.id === data.id) {
      throw new Error("You cannot edit your own user information through this endpoint.")
    }
