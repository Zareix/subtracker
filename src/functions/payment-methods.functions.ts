...
import { paymentMethods, subscriptions, usersToSubscriptions } from "~/lib/db/schema"
  ...
  .handler(async ({ data }) => {
    ...
    await runTransaction(db, async (tx) => {
      ...
      for (const sub of subs) {
        await tx.delete(usersToSubscriptions).where(eq(usersToSubscriptions.subscriptionId, sub.id))
