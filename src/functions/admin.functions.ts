...
export const adminCleanUpFiles = createServerFn({ method: "POST" }).handler(async () => {
  await requireAdmin()
  const filesInUse = (
    await Promise.all([
...
export const adminMigrateImagesToS3 = createServerFn({
  ...
  for (const sub of localSubscriptions) {
    ...
    await db.update(subscriptions).set({ image: newImage }).where(eq(subscriptions.id, sub.id))
  }

  for (const pm of localPaymentMethods) {
    if (!pm.image) continue
    const newImage = await migrateImageToS3(pm.image)
    await db.update(paymentMethods).set({ image: newImage }).where(eq(paymentMethods.id, pm.id))
  }
...
export const adminExportData = createServerFn({ method: "POST" }).handler(async () => {
  await requireAdmin()
  const [allSubscriptions, allPaymentMethods, allCategories, allUsers, allUserToSubscriptions] =
    await Promise.all([
      db.query.subscriptions.findMany(),
      db.query.paymentMethods.findMany(),
  ...
  .handler(async ({ data }) => {
    ...
    await runTransaction(db, async (tx) => {
      ...
      if (data.categories && data.categories.length > 0) {
        await tx.insert(categories).values(data.categories.filter((c) => c.id !== 1))
      }
