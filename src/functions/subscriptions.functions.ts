...
import { Currencies, type Currency, DEFAULT_BASE_CURRENCY, SCHEDULES } from "~/lib/constant"
...
export const getAllSubscriptionsOfUser = createServerFn({
  ...
  .handler(async ({ data: { userId, baseCurrency } }) => {
    ...
    return rows
      ...
      .filter((subscription) => subscription.users.some((user) => user.id === userId))
      .map((subscription) => {
...
export const editSubscription = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const subscription = await runTransaction(db, async (tx) => {
      ...
      await tx.delete(usersToSubscriptions).where(eq(usersToSubscriptions.subscriptionId, data.id))
      await Promise.all(
        data.payedBy.map((userId) =>
          tx.insert(usersToSubscriptions).values({
            userId,
...
export type SubscriptionItem = Awaited<ReturnType<typeof getAllSubscriptionsOfUser>>[number]
