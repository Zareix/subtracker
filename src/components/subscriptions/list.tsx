...
export const SubscriptionList = ({ subscriptions }: Props) => {
  ...
  if (subs.length === 0) {
    return <div className="text-center text-muted-foreground">{m.subscription_list_empty()}</div>
  }

  const now = startOfToday()
  const previousSubOfThisMonth = subs
    .filter((s) => isThisMonth(s.previousPaymentDate) && isBefore(s.previousPaymentDate, now))
    .toSorted((a, b) => compareAsc(a.previousPaymentDate, b.previousPaymentDate))
  ...
  return (
    <>
      ...
      {subs.map((subscription) => (
        <React.Fragment key={subscription.id}>
          <SubscriptionListItem subscription={subscription} userBaseCurrency={userBaseCurrency} />
    ...
    >
      <CardContent>
        <div className="flex items-center gap-2">
          ...
          <div className="flex grow flex-col gap-1">
            ...
            <div className="flex items-center gap-1 text-sm text-muted-foreground">
              ...
              {formatNextPaymentDate(
                isPrevious ? subscription.previousPaymentDate : subscription.nextPaymentDate,
              )}
            </div>
          </div>
          ...
          <DropdownMenu>
            ...
            />
            <DropdownMenuContent className="mr-2 w-32" onClick={(e) => e.stopPropagation()}>
              <DropdownMenuItem onClick={() => setIsOpen({ ...isOpen, edit: true })}>
                <EditIcon />
                <span>{m.subscription_list_edit()}</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setIsOpen({ ...isOpen, duplicate: true })}>
                <CopyPlusIcon />
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-2 pt-1 text-base text-foreground/80 md:gap-x-6">
              ...
              setFilters({
                paymentMethods:
                  filters.paymentMethods.length > 1
                    ...
                    : filters.paymentMethods[0] === subscription.paymentMethod.id
                      ? undefined
          ...
          >
            <CategoryIcon icon={subscription.category.icon} className="size-4 text-primary" />
          ...
          <button
            ...
            onClick={() =>
              setFilters({
                schedule: filters.schedule === subscription.schedule ? null : subscription.schedule,
              })
            }
          >
