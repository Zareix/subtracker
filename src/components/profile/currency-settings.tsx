...
export const CurrencySettings = () => {
  ...
  const updateCurrencyMutation = useMutation({
    mutationFn: (newCurrency: Currency) => authClient.updateUser({ baseCurrency: newCurrency }),
    onSuccess: (res) => {
      if (res.error) {
    ...
    onError: (err) => toast.error(err.message ?? m.profile_currency_update_failed()),
  })

  if (isSessionLoading || !user) {
    return (
      <section>
        <h2 className="mb-4 text-2xl font-bold">{m.profile_currency_title()}</h2>
  ...
  return (
    <section>
      ...
      <Select
        ...
        onValueChange={(value) => updateCurrencyMutation.mutate(value as Currency)}
        items={currencies}
      >
