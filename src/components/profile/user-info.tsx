...
export const UserInfoForm = ({ user }: Props) => {
  ...
  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold">{m.profile_info_title()}</h2>
      <form
        onSubmit={(e) => {
          e.preventDefault()
          e.stopPropagation()
      ...
      >
        <FieldGroup>
              ...
              {(field) => {
                ...
                return (
                  <Field data-invalid={isInvalid} className="col-span-10">
                    <FieldLabel htmlFor="profile-name">{m.settings_form_name()}</FieldLabel>
                    <Input
                      id="profile-name"
                      name={field.name}
            ...
            {(field) => {
              ...
              return (
                <Field data-invalid={isInvalid}>
                  <FieldLabel htmlFor="profile-email">{m.admin_form_email()}</FieldLabel>
                  <Input
                    id="profile-email"
                    name={field.name}
                    type="email"
          ...
          <Field>
            <FieldLabel htmlFor="profile-role">{m.admin_form_role()}</FieldLabel>
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={editMutation.isPending} className="w-full sm:w-auto">
              {editMutation.isPending ? m.profile_info_updating() : m.profile_info_update()}
            </Button>
          </div>
        </FieldGroup>
      </form>
