...
export const ApiKeys = ({ userId }: Props) => {
  ...
  const createMutation = useMutation({
    mutationFn: (data: z.infer<typeof schema>) => authClient.apiKey.create({ name: data.name }),
    onSuccess: (data) => {
  ...
  return (
    <section>
      <h3 className="mb-4 text-lg font-semibold">{m.profile_api_keys_title()}</h3>

      {lastCreatedKey && (
        <Card className="mb-6 border-green-200 bg-green-50 py-4 dark:border-green-800 dark:bg-green-950">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center gap-2 rounded bg-green-100 p-3 dark:bg-green-900">
              <code className="flex-1 font-mono text-sm break-all text-green-800 dark:text-green-200">
                {lastCreatedKey.key}
              </code>
              <Button
                variant="ghost"
      ...
      {apiKeysQuery.isLoading ? (
        ...
        <p className="text-muted-foreground">{m.profile_api_keys_no_created()}</p>
      ) : (
            ...
            {(field) => {
              ...
              return (
                <Field data-invalid={isInvalid}>
                  <FieldLabel htmlFor="api-key-name">{m.profile_api_keys_name()}</FieldLabel>
                  <Input
                    id="api-key-name"
                    name={field.name}
