...
export const EditCreateForm = ({
  ...
  const editMutation = useMutation({
    mutationFn: (data: z.infer<typeof schema> & { id: number }) => editCategory({ data }),
    onSuccess: () => {
          ...
          {(field) => {
            ...
            return (
              <Field data-invalid={isInvalid}>
                <FieldLabel htmlFor="category-name">{m.settings_form_name()}</FieldLabel>
                <Input
                  id="category-name"
                  name={field.name}
          ...
          {(field) => {
            ...
            return (
              <Field data-invalid={isInvalid} className="flex flex-col">
                <FieldLabel htmlFor="category-icon">{m.settings_icon_label()}</FieldLabel>
                            ...
                            {filteredIconNames.map((name) => (
                              ...
                              >
                                ...
                                <CheckIcon
                                  className={cn(
                                    "ml-auto",
                                    name === field.state.value ? "opacity-100" : "opacity-0",
                                  )}
                                />
                              </CommandItem>
                            ))}
