...
import { createPaymentMethod, editPaymentMethod } from "~/functions/payment-methods.functions"
...
export const EditCreateForm = ({
  ...
  const editMutation = useMutation({
    mutationFn: (data: z.infer<typeof schema> & { id: number }) => editPaymentMethod({ data }),
    onSuccess: () => {
            ...
            {(field) => {
              ...
              return (
                <Field data-invalid={isInvalid} className="col-span-8">
                  <FieldLabel htmlFor="pm-name">{m.settings_form_name()}</FieldLabel>
                  <Input
                    id="pm-name"
                    name={field.name}
